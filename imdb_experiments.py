...
def test_temperature_scaling():
    ...
    # dataset metricxs
    print(dataset_metrics((train_text, train_labels)))
    print(dataset_metrics((validation_text, validation_labels)))
    print(dataset_metrics((test_text, test_labels)))
    print(dataset_metrics((unlabeled_text, unlabeled_labels)))
    ...
    print('Size of validation: ', len(validation_dataset))
    print('Size of test: ', len(test_dataset))

    model = TextClassificationModel(768, 2)
    model = model_training(model, device, 50, train_dataloader, criterion)

    # determine parameter T for temperature scaling
    T = calibrate_temperature_scaling(validation_dataloader, device, model)
    temp_scaling = TemperatureScaling(temp=T)

    # examine calibration on train set
    train_pre_softmax_probs, train_post_softmax_probs, train_predicted_labels, train_true_labels = get_model_predictions(train_dataloader, device, model)
    train_accuracy = accuracy_score(train_true_labels, train_predicted_labels)
    print('Train accuracy: ', train_accuracy)

    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/temp_scaling_train_initial_calibration.jpg')

    # recalibrate and examine new calibration on train set
    train_post_softmax_probs = temp_scaling.predict(train_pre_softmax_probs)
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/temp_scaling_train_after_calibration.jpg')

    # examine calibration on test set
    test_pre_softmax_probs, test_post_softmax_probs, test_predicted_labels, test_true_labels = get_model_predictions(test_dataloader, device, model)
    test_accuracy = accuracy_score(test_true_labels, test_predicted_labels)
    print('Test accuracy: ', test_accuracy)

    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/temp_scaling_test_initial_calibration.jpg')

    # recalibrate and examine new calibration on test set
    test_post_softmax_probs = temp_scaling.predict(test_pre_softmax_probs)
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/temp_scaling_test_after_calibration.jpg')
...
def test_histogram_binning():
    ...
    # determine parameter T for temperature scaling
    histogram_binning = calibrate_histogram_binning(validation_dataloader, device, model)
    ...
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/histogram_train_initial_calibration.jpg')

    # recalibrate and examine new calibration on train set
    train_post_softmax_probs = histogram_binning.predict(train_pre_softmax_probs)
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/histogram_train_after_calibration.jpg')
    ...
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/histogram_test_initial_calibration.jpg')

    # recalibrate and examine new calibration on test set
    test_post_softmax_probs = histogram_binning.predict(test_pre_softmax_probs)
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/histogram_test_after_calibration.jpg')
...
def test_isotonic_regression():
    ...
    # determine parameter T for temperature scaling
    isotonic_regression = calibrate_isotonic_regression(validation_dataloader, device, model)
    ...
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/isotonic_train_initial_calibration.jpg')

    # recalibrate and examine new calibration on train set
    train_post_softmax_probs = isotonic_regression.predict(train_pre_softmax_probs[:,1])
    temp = np.zeros((train_post_softmax_probs.shape[0], 2))
    temp[:, 1] = train_post_softmax_probs
    temp[:, 0] = 1 - train_post_softmax_probs
    train_post_softmax_probs = temp
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/isotonic_train_after_calibration.jpg')
    ...
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/isotonic_test_initial_calibration.jpg')

    # recalibrate and examine new calibration on test set
    test_post_softmax_probs = isotonic_regression.predict(test_pre_softmax_probs[:,1])
    temp = np.zeros((test_post_softmax_probs.shape[0], 2))
    temp[:, 1] = test_post_softmax_probs
    temp[:, 0] = 1 - test_post_softmax_probs
    test_post_softmax_probs = temp
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/isotonic_test_after_calibration.jpg')

def test_beta_calibration():
    ...
    # determine parameter T for temperature scaling
    beta_calibration = calibrate_beta_calibration(validation_dataloader, device, model)
    ...
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/beta_train_initial_calibration.jpg')

    # recalibrate and examine new calibration on train set
    train_post_softmax_probs = beta_calibration.predict(train_pre_softmax_probs[:,1])
    temp = np.zeros((train_post_softmax_probs.shape[0], 2))
    temp[:, 1] = train_post_softmax_probs
    temp[:, 0] = 1 - train_post_softmax_probs
    train_post_softmax_probs = temp
    plot_calibration_curve(train_true_labels, train_post_softmax_probs, 'temperature_scaling_test4/beta_train_after_calibration.jpg')
    ...
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/beta_test_initial_calibration.jpg')

    # recalibrate and examine new calibration on test set
    test_post_softmax_probs = beta_calibration.predict(test_pre_softmax_probs[:,1])
    temp = np.zeros((test_post_softmax_probs.shape[0], 2))
    temp[:, 1] = test_post_softmax_probs
    temp[:, 0] = 1 - test_post_softmax_probs
    test_post_softmax_probs = temp
    plot_calibration_curve(test_true_labels, test_post_softmax_probs, 'temperature_scaling_test4/beta_test_after_calibration.jpg')
